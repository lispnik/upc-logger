...
- **Grouped or not:** a row per scan to begin with — the log as it was recorded. The strip above the search bar switches to a row per *run*, adjacent scans of one code drawn as `3 x 012345678905`. The chart always bins by individual scan, whichever view is on, which is the reason the times are kept.
