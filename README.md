...
- **A photo of a whole set:** scan a dozen tins, then photograph the dozen. The ⓘ on a merged row offers "Photograph all 12 as a set", and that one picture belongs to the run rather than to any scan in it — so it does not split the run, and in **Every scan** it appears on each of the twelve rows. Two runs of the same code photographed separately stay separate, since one picture cannot cover scans it never saw.
- **The photo itself:** tapping the thumbnail on a row opens the picture full screen, with **Replace** and **Delete** beside Close. A row showing the set's picture replaces or deletes the set's; a row with its own replaces or deletes that one. Deleting removes the file from `Documents/photos` as well as the reference, and replacing removes the one it supersedes, so nothing piles up unreferenced. Tapping anywhere else on the row still sets the count.
